...
arms_aggregated.rename(columns={'Recipient': 'country', 'Year of order': 'year', 
                                'Number delivered': 'total_arms_delivered', 
                                'SIPRI TIV for total order': 'total_arms_value'}, inplace=True)
...
def calculate_pearson_correlation(df, col1, col2):
    filtered_df = df[[col1, col2]].dropna()  # Ensure both columns have same length by dropping NaN
    if not filtered_df.empty:
        corr, p_value = pearsonr(filtered_df[col1], filtered_df[col2])
        print(f"Pearson correlation coefficient between {col1} and {col2}: {corr:.3f}")
        print(f"P-value of the correlation: {p_value:.3g}\n")
    else:
        print(f"No valid data for correlation between {col1} and {col2} due to NaN values.")

# Example usage with the merged and lagged global data
calculate_pearson_correlation(merged_data, 'No_Incidents', 'total_arms_value_lag1')
calculate_pearson_correlation(merged_data, 'No_death', 'total_arms_value_lag1')
calculate_pearson_correlation(merged_data, 'No_Incidents', 'total_arms_value_lag2')
calculate_pearson_correlation(merged_data, 'No_death', 'total_arms_value_lag2')
